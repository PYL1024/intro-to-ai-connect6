...
public class MoveGenerator {
    ...
    public List<int[]> generateMoves(PieceColor myColor, PieceColor oppColor) {
        ...
        if (urgentDefense && !defensePositions.isEmpty()) {
            ...
            // 正常模式：按分值组合
            moves = generateOffensiveMoves(scoredPositions, myColor);
        }
    ...
    /**
     * 自适应候选上限：随对局阶段与威胁压力调整。
     * Early: 40；Mid: 35；Late: 30；存在高压时小幅放宽。
     */
    private int computeAdaptiveCandidateCap() {
        int pieceCount = board.getPieceCount();
        int cap;
        if (pieceCount < 30) cap = 40;
        else if (pieceCount < 80) cap = 35;
        else cap = 30;

        try {
            int pressure = 0;
            pressure = threatEvaluator.getCriticalThreatCount() * 2 + threatEvaluator.getThreatCount();
            if (pressure >= 3) cap = Math.min(45, cap + 5);
